...
test('toolbar buttons have accessible labels and focus states', async ({ page }) => {
  ...
  await expect(coverZoomBtn).not.toHaveClass(/focus:outline-none/);
  ...
  await expect(coverCropBtn).not.toHaveClass(/focus:outline-none/);
  ...
  await expect(coverRemoveBtn).not.toHaveClass(/focus:outline-none/);
});
