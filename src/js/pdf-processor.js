...
export class PDFProcessor {
  ...
  async loadPDF(file, onProgress = null) {
    ...
    try {
      ...
      console.error('PDF loading error:', error.message || 'An error occurred');
  ...
  async renderPage(pageNum, onProgress = null) {
    ...
    try {
      ...
      console.error(`Failed to render page ${pageNum}:`, error.message || 'An error occurred');
  ...
  canvasToDataURL(canvas, format = 'image/jpeg', quality = 0.85) {
    try {
      ...
      console.warn(`Failed to generate ${format}, falling back to PNG:`, error.message || 'An error occurred');
