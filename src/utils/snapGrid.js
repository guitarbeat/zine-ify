...
function savePos(id, left, top) {
  try { localStorage.setItem('zine-card3-' + id, JSON.stringify({ left, top })); } catch (_e) { void _e; }
}
function loadPos(id) {
  try { const r = localStorage.getItem('zine-card3-' + id); return r ? JSON.parse(r) : null; } catch (_e) { void _e; return null; }
}
