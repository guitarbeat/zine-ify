...
## 2026-06-25 - Icon Accessibility with Ligatures
**Learning:** Material Symbol ligatures (like `<span class="material-symbols-outlined">zoom_in</span>`) can be incorrectly read by screen readers as the literal text "zoom in" if used as an icon inside a button. Icon-only buttons also fail accessibility without a proper label.
**Action:** Always add `aria-hidden="true"` to icon ligatures and ensure the parent interactive element (e.g. `<button>`) has a descriptive `aria-label` or `title` to provide context.
