...
## 2026-10-27 - Material Symbols Accessibility
**Learning:** Material Symbols ligatures (e.g., `<span class="material-symbols-outlined">cloud_upload</span>`) are announced by screen readers as the raw text ("cloud upload") if not hidden, which can be confusing and redundant when the button already has an `aria-label` or surrounding text.
**Action:** Always add `aria-hidden="true"` to Material Symbols spans to ensure they are treated as purely decorative by assistive technologies.
