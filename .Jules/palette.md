...
## 2026-10-24 - Contextual Icon Buttons & Ligature Hiding
**Learning:** Icon-only action buttons (e.g., zoom, crop) inside dynamic repeating lists (like grid cells) lack context if their `aria-label` is generic. Furthermore, Material Symbols ligatures (e.g., `<span class="material-symbols-outlined">zoom_in</span>`) are announced as raw text ("zoom underscore in") by screen readers if not hidden.
**Action:** Ensure icon-only buttons receive contextual `aria-label`s (e.g., "Quick preview Page 1") and always apply `aria-hidden="true"` to the icon element itself. Use a centralized helper method for dynamic generation to keep this logic consistent.
