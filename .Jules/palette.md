...
## 2024-05-22 - Icon-Only Button Accessibility and Material Symbols Ligatures
**Learning:** Material Symbols ligatures (e.g., `<span class="material-symbols-outlined">icon</span>`) are announced by screen readers as the raw ligature text (e.g., "cloud upload") unless `aria-hidden="true"` is added. Additionally, icon-only action buttons require contextual `aria-label` attributes to ensure screen reader users understand the button's action and target (e.g., "Quick Preview Page 1" instead of just "Quick Preview").
**Action:** Always add `aria-hidden="true"` to Material Symbols ligatures and ensure icon-only buttons have descriptive `aria-label` attributes that include context.
