...
## 2026-10-24 - Material Symbols Accessibility
**Learning:** Screen readers announce raw ligature text for Material Symbols (e.g., "cloud_upload", "rotate_right"), creating a confusing experience for users relying on assistive technology.
**Action:** Always add `aria-hidden="true"` to `<span class="material-symbols-outlined">` and other icon elements, and ensure their parent interactive element (like a `<button>`) has an appropriate `aria-label` or visible text.
