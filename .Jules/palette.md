...
## 2024-05-22 - Accessibility for Icon-only Buttons and Ligatures
**Learning:** Icon-only action buttons relying solely on `title` tooltips are inaccessible to screen readers without a contextual `aria-label`. Furthermore, using material symbol ligatures inside elements without `aria-hidden="true"` can cause screen readers to read the raw ligature text (e.g., "zoom in").
**Action:** Always include contextual `aria-label` attributes on icon-only buttons, and apply `aria-hidden="true"` to any material symbols or icon ligatures. Ensure that when elements are generated dynamically, `aria-label` attributes are added directly when the elements are created in the DOM.
