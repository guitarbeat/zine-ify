...
## 2025-02-23 - Keyboard Event Normalization
**Learning:** Assuming `e.key` will always match the case of the key pressed (e.g., 'P') is risky because `CapsLock` or shift keys can affect it.
**Action:** Always normalize keys with `.toLowerCase()` when checking for shortcuts to ensure `Shift+P` or `CapsLock` don't break the shortcut handler.
