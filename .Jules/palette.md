...
## 2026-06-25 - Tooltip and ARIA Labels on Toolbar Buttons
**Learning:** The hover action toolbar on grid pages for functions like Zoom, Crop, and Remove had `title` tooltips but were missing `aria-label` attributes for screen readers, meaning non-sighted users wouldn't know what these icon-only buttons do.
**Action:** Add `aria-label` attributes that mirror or expand upon the `title` tooltip values on all icon-only action buttons.
