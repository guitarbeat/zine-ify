...
## 2025-02-28 - High-Contrast Focus & ARIA Icons
**Learning:** High-contrast focus outlines combined with important overrides (`!bg-yellow-300`) ensure keyboard navigability on dark action buttons. Furthermore, Material Symbols ligatures (e.g., `cloud_upload`) read aloud poorly by screen readers and need `aria-hidden="true"` when paired with visible text.
**Action:** Always apply explicit, high-contrast focus states (e.g., `focus-visible:outline-dashed`) on action buttons, and explicitly hide icon ligatures from screen readers using `aria-hidden="true"`.
