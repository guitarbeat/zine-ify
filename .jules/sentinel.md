...
## 2025-02-14 - [XSS via Dynamically Generated Windows]
**Vulnerability:** A dynamically generated print window created via `window.open('', '_blank')` and populated using `document.write()` lacked a Content Security Policy (CSP). The new window did not inherit the parent document's CSP, leaving it fully exposed to XSS if malicious content was included in the injected HTML.
**Learning:** Newly spawned empty windows do not inherit the parent document's `Content-Security-Policy`. When populating these windows dynamically with `document.write()`, they operate with the default, permissive browser policies.
**Prevention:** Always inject a strict `<meta http-equiv="Content-Security-Policy">` tag into the `<head>` of any HTML string being written to a new window via `document.write()` to ensure the generated document operates under a secure policy.
