...
## 2025-02-14 - [Insufficient Content Security Policy]
**Vulnerability:** The application's Content Security Policy (CSP) lacked `object-src`, `base-uri`, and `form-action` directives, leaving it vulnerable to object tag attacks (like Flash/PDF embedding) and base tag hijacking.
**Learning:** A robust defense-in-depth strategy requires tightening all possible execution vectors. A CSP must explicitly block dangerous resources that aren't used (like `<object>`, `<embed>`, `<applet>`), and lock down base URIs, rather than defaulting to `default-src`. Additionally, adding `noopener,noreferrer` to `window.open` for local blank pages (e.g., `window.open('', '_blank')`) breaks functionality because it returns `null`.
**Prevention:** Add `object-src 'none'`, `base-uri 'self'`, and `form-action 'self'` to the CSP to strictly limit dangerous, unused capabilities. Do not apply `noopener` when manipulating local dynamically generated windows.
