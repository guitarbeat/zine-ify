...
## 2025-02-14 - [XSS via Dynamically Generated Windows without CSP]
**Vulnerability:** The application opened a new window (`window.open('', '_blank')`) and used `document.write()` to generate a print layout containing images derived from uploaded PDFs. The dynamically generated HTML in the new window lacked a Content Security Policy (CSP). While the parent document had a CSP, the new window does not inherit it, exposing the generated document to potential Cross-Site Scripting (XSS).
**Learning:** Windows created via `window.open` and populated with `document.write()` act as independent browsing contexts. They do not automatically inherit the CSP of the originating page. If any user-controlled content (even filenames, metadata, or data URIs) is reflected into these dynamic windows, the lack of CSP creates an XSS vulnerability vector.
**Prevention:** When dynamically generating HTML documents in a new window via `document.write()`, explicitly inject a strict `<meta http-equiv="Content-Security-Policy">` tag into the `<head>` of the generated document to enforce the same or stricter security policies as the parent application.
