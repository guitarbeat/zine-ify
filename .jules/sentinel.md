...
## 2025-02-14 - [Missing CSP in window.open Dynamic Documents]
**Vulnerability:** The application opened a new window for printing using `window.open('', '_blank')` and directly wrote HTML content into it. Because the new window is dynamically generated and not loaded from a URL, it doesn't inherit the parent document's Content Security Policy (CSP), leaving it vulnerable to XSS if any untrusted data is included in the print layout.
**Learning:** Dynamically generated documents in new windows via `window.open` and `document.write` are fresh contexts that bypass the parent page's CSP. They require their own explicit CSP.
**Prevention:** Always explicitly inject a `<meta http-equiv="Content-Security-Policy">` tag into the `<head>` of any dynamically generated HTML document written to a new window or iframe, enforcing a strict policy like `default-src 'none'; img-src 'self' data: blob:; style-src 'unsafe-inline';`.
