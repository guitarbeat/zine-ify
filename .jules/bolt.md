...
**Action:** Use `OffscreenCanvas` instead of standard DOM canvases where available (e.g., in PDF.js rendering loops and blank page generation) to allow asynchronous rendering without blocking the main UI thread. Ensure a fallback to `document.createElement('canvas')` for unsupported environments. Use `.convertToBlob()` for OffscreenCanvases in place of standard canvas `.toBlob()`.## 2026-04-13 - Sliding Window Concurrency for PDF Processing
**Learning:** Using sequential `await` loops or batched `Promise.all` for processing multiple PDF pages causes significant performance bottlenecks and UI stuttering during large imports.
**Action:** Implement a sliding window worker pool pattern using `Promise.race` and a `Set` of active promises with a fixed concurrency limit (e.g., 4). This maintains a constant stream of processing, preventing UI stuttering and utilizing resources more efficiently.
